...
export class NotificationService {
  ...
  private static async isDuplicate(
    ...
    try {
      ...
      const stored = await AsyncStorage.getItem(
        `${NOTIFICATION_STORAGE.LAST_SENT}:${lastSentKey}`
      );
  ...
  private static async saveLastSent(
    type: NotificationType,
    data?: any
  ): Promise<void> {
    try {
  ...
  static async scheduleDailyReminder(): Promise<string | null> {
    try {
      ...
      const [hours, minutes] = preferences.dailyReminderTime
        .split(":")
        .map(Number);
      await this.cancelDailyReminder();
  ...
  static async cancelDailyReminder(): Promise<void> {
    try {
      const id = await AsyncStorage.getItem(
        NOTIFICATION_STORAGE.DAILY_REMINDER_ID
      );
      if (id) {
  ...
  static async cancelGoalDeadlineReminder(
